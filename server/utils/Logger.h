...
class Logger {
public:
    // מחזירה את מופע הסינגלטון של ה-Logger.
    static Logger& instance();

    // פותח (או יוצר) את קובץ הלוג. יש לקרוא פעם אחת בהפעלה.
    void init(const string& filePath = "iris_server.log");
    ...
    // כותב שורה עם תאריך/שעה ורמת לוג למסוף ולקובץ (אם פתוח).
...
private:
    ...
    // מחזירה מחרוזת עם שם רמת הלוג.
    static string levelStr(LogLevel level);

    // מחזירה מחרוזת עם התאריך והשעה הנוכחיים בפורמט "YYYY-MM-DD HH:MM:SS.mmm".
